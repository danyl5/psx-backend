...
const roundToTwoDecimals = (value) =>
  Math.round((sanitizeNumber(value) + Number.EPSILON) * 100) / 100;
...
const ensureDefaultPortfolioTab = async (userId) => {
  ...
  await Portfolio.updateMany(
    { user: userId, portfolionumber: { $exists: false } },
    {
      $set: {
        portfolionumber: 1,
        portfolioName: nameByNumber.get(1) || "Portfolio 1",
      },
    },
  );
  await Promise.all(
    tabs.map((tab) =>
      Portfolio.updateMany(
        {
          user: userId,
          portfolionumber: tab.portfolionumber,
          portfolioName: { $exists: false },
        },
...
const normalizePortfolioRows = (rows) =>
  rows.map((row) => ({
    ...
    dividendAfterTax:
      row.dividendAfterTax ?? row.dividendTotal ?? row.dividend ?? 0,
  }));

export const applyPurchaseToPortfolio = async (
  userId,
  { script, quantity, price, portfolionumber },
) => {
  ...
  const portfolioTab = tabs.find(
    (tab) => tab.portfolionumber === normalizedPortfolioNumber,
  );
  if (!portfolioTab) {
  ...
  if (existing) {
    ...
    existing.avgPrice =
      totalShares > 0 ? roundToTwoDecimals(totalCost / totalShares) : 0;
    await existing.save();
    return existing;
  }

  const matchedScript = await findScriptBySymbol(normalizedScript);
  const maxOrderRow = await Portfolio.findOne(
    getPortfolioRowFilter(userId, normalizedPortfolioNumber),
  )
    .sort({ orderNumber: -1 })
    .select("orderNumber");
...
export const getPortfolioTabs = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while fetching portfolio tabs" });
  }
};
...
export const createPortfolioTab = async (req, res) => {
  try {
    const { name = "" } = req.body;
    const tabs = await ensureDefaultPortfolioTab(req.user._id);
    const nextPortfolioNumber =
      Math.max(...tabs.map((tab) => tab.portfolionumber), 0) + 1;
    ...
    return res
      .status(500)
      .json({ message: "Server error while creating portfolio tab" });
  }
};
...
export const updatePortfolioTab = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while updating portfolio tab" });
  }
};
...
export const deletePortfolioTab = async (req, res) => {
  try {
    ...
    if (tabs.length <= 1) {
      return res
        .status(400)
        .json({ message: "You must keep at least one portfolio" });
    }
    ...
    const remainingTabs = await PortfolioTab.find({ user: req.user._id })
      .sort({ portfolionumber: 1 })
      .lean();
    return res
      .status(200)
      .json({ message: "Portfolio deleted successfully", tabs: remainingTabs });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Server error while deleting portfolio tab" });
  }
};
...
export const getPortfolioRows = async (req, res) => {
  try {
    ...
    const symbolsMissingSector = [
      ...new Set(
        rowsWithDividends
          .filter((row) => !row.sector && row.script)
          .map((row) => row.script),
      ),
    ];
    ...
    if (!symbolsMissingSector.length) {
      return res.status(200).json({
        rows: normalizePortfolioRows(rowsWithDividends).map((row) => ({
          ...row,
          portfolioName: row.portfolioName || tab.name,
        })),
        portfolionumber,
      });
    }
    ...
    return res
      .status(500)
      .json({ message: "Server error while fetching portfolio" });
  }
};
...
export const addPortfolioRow = async (req, res) => {
  try {
    ...
    const row = await Portfolio.create({
      ...
      dividendAfterTax: sanitizeNumber(
        dividendAfterTax || dividendTotal || dividend,
      ),
      orderNumber:
    ...
    return res
      .status(500)
      .json({ message: "Server error while adding portfolio row" });
  }
};
...
export const updatePortfolioRow = async (req, res) => {
  try {
    ...
    if (dividendTotal !== undefined || dividend !== undefined) {
      row.dividendTotal = sanitizeNumber(
        dividendTotal !== undefined ? dividendTotal : dividend,
      );
    }
    ...
    return res
      .status(500)
      .json({ message: "Server error while updating portfolio row" });
  }
};
...
export const reorderPortfolioRows = async (req, res) => {
  try {
    ...
    if (rows.length !== uniqueRowIds.length) {
      return res
        .status(400)
        .json({ message: "One or more portfolio rows are invalid" });
    }
    ...
    return res
      .status(500)
      .json({ message: "Server error while reordering portfolio rows" });
  }
};
...
export const deletePortfolioRow = async (req, res) => {
  try {
    const { id } = req.params;
    const row = await Portfolio.findOneAndDelete({
      _id: id,
      user: req.user._id,
    });
    ...
    return res
      .status(200)
      .json({ message: "Portfolio row deleted successfully" });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Server error while deleting portfolio row" });
  }
};
