...
export const getAllShariahStocks = async (req, res) => {
  try {
    const data = await fetchAllShariaStocks();

    return res.status(200).json(data);
  } catch (error) {
    console.error("Error in getAllShariahStocks controller:", error);
    return res
      .status(500)
...
export const getUpperCapScanner = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to generate upper cap scanner results." });
  }
};
